DROP FUNCTION fetch_entities_within_view(
    DOUBLE PRECISION,
    DOUBLE PRECISION,
    DOUBLE PRECISION,
    DOUBLE PRECISION,
...
BEGIN
    ...
    filtered_entities AS (
        ...
        WHERE
            ST_Intersects(
                ae.web_mercator_location,
                ST_MakeEnvelope(input_xmin, input_ymin, input_xmax, input_ymax, 3857)
            )
            -- User filters
            AND (ae.category_id = ANY(active_categories_ids))
            AND (array_length(required_tags_ids, 1) = 0 OR required_tags_ids <@ ae.tags_ids)
            AND NOT (ae.tags_ids && excluded_tags_ids)
            -- Enum constraints
            AND (
                ...
                (
                    SELECT bool_and(
                        ae.enums->key ?| array(SELECT jsonb_array_elements_text(value))
                    )
                    FROM jsonb_each(enum_constraints) AS constraints(key, value)
                    WHERE key IS NOT NULL AND ae.enums ? key
                )
            )
    ),
    parent_entities AS (
    ...
    combined_entities AS (
        SELECT * FROM filtered_entities fe 
            -- either
            LEFT JOIN parent_entities pe ON fe.parent_id = pe.id
            WHERE pe.id IS NULL
            -- or
            --WHERE fe.parent_id NOT IN (
            --    SELECT id
            --    FROM parent_entities
            --)
            --
        UNION
    ...
    SELECT
        ...
        ce.parent_id IN (SELECT pe.id FROM parent_entities pe) AS is_parent_included,
